...
- §4.28.3 で閉状態に `invisible` を足したが、同要素の `transition-all` は
  ...
  実際に動かしたいのは位置・幅・影だけなので `transition-[transform,width,box-shadow]` と
  明示列挙し、visibility は即座に切り替わるようにした（生成 CSS で
  `transition-property:transform,width,box-shadow` を確認）。
...
- E2E のハイドレーション競合を回避: `page.goto` は `load` で解決する一方 `MobileNavToggle` は
  ...
  覆い隠すため、検出網としてはむしろ有害）。クリック前に `aria-expanded` の値を待つようにした。
