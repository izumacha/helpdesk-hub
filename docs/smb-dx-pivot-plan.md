...
- **索引の根拠の写しを 3 → 1 箇所へ**: 索引を足した理由を `schema.prisma` / `migration.sql` /
  ...
  あわせて索引の効き方の記述を実測に即した条件付きへ改めた（シードデータ規模では
  再オープン率の集約でも Index Scan が選ばれてコストが下がった一方、20 万行規模では
  hash / merge join が選ばれて参照されない。**索引の存在理由はチケット詳細の履歴取得**）。
- **適用済みマイグレーションを後から編集しない**: 3 巡目の是正でマイグレーション本文の
  ...
  運用注意だけを残し、以後は触らない方針にした。**このブランチを 013a0f1 時点で適用済みの環境は、
  DB を作り直すか `prisma migrate resolve` で記録し直すこと。**
