...
### 3.4 チケット種別（リクエスト/インシデント）の扱い（2026-09-09 決定）

- **方針: 起票時に依頼者へ種別（リクエスト/インシデント）の選択を求めない。分類が必要な場合は、対応者（agent）が起票後に行う。**
- 根拠:
  1. **用語簡素化方針（§3.1）と衝突する** — 「リクエスト/インシデント」は ITIL 由来の専門用語で、ペルソナ（§1「専門用語はピンと来ない」）に正しい選択を求められない。誤分類の訂正コストが対応者側に乗るだけで、起票時に選ばせる利点が無い。
  2. **Phase 1 の簡易フォーム仕様に反する** — Lite の起票フォームは「件名 / 内容 / 期限日」に絞ることが完了済み仕様（Phase 1）。必須入力の追加はハードルを上げ、北極星「最短で Excel から卒業できる、最小単位のヘルプデスク」（§0）を損なう。
  3. **メール / LINE 取り込み（Phase 2）と両立しない** — 転送・LINE 送信でそのままチケット化する入口には起票時の選択 UI が存在しないため、種別を起票時必須にすると取り込み経路すべてで欠損する。
  4. **事後分類は確立済みのパターン** — 取り込みチケットは未分類で起票し、担当者が後からカテゴリ・拠点を設定する運用を §4.8 で導入済み（`updateTicketCategory` / `CategorySelect`）。種別も同じ扱いに揃える。
- 将来、種別軸が本当に必要になった場合の条件:
  - 先に本計画書（この節と §3.1 の用語表）を PR で改訂してから実装に着手する。
  - 形は **Pro 限定・対応者による事後分類・履歴記録あり**（カテゴリと同じ扱い）とし、依頼者の入力項目にはしない。
  - `Ticket` への新規フィールド追加より、既存の `Category`（業種テンプレで初期投入・事後変更 UI と履歴・監査あり）への搭載を優先して検討する。
