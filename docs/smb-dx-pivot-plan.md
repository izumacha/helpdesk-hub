...
- [ ] 「対応すると依頼者にメールで返信が届く」を既定動作に（依頼者がアプリにログインしなくても完結）

### Phase 3 — オンボーディング & 業種テンプレ（3 週間）

- [ ] テナント作成時に **業種テンプレ**（製造業 / 飲食 / 介護 / 不動産 / 士業 / 卸売 など）でカテゴリと「よくある質問」を初期投入
- [ ] **Excel インポート**: 既存の問い合わせ管理 Excel を CSV にして取り込み（最低限の列マッピングウィザード）
- [ ] チュートリアル動画リンク・サンプルチケット
- [ ] ヘルプセンター（このリポジトリ内に Next.js で同梱、SSG）
...
- [ ] Slack / Chatwork / Microsoft Teams 通知 Adapter
