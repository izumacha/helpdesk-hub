...
- **終息ステータスの写しを 6 → 1 箇所へ**。`'Resolved' / 'Closed'` の組が Prisma アダプタ・
...
- **「必ず 0 件」の組み合わせを作る入口が 2 つあり、方針が食い違っていた**。タブ切替は
  ...
  外すキーの判定を `filtersClearedByStatusChange` に 1 か所だけ置いて両方の入口で共有する。
...
**残っている課題（記録）**: `?open=1` は一覧・CSV エクスポート・ワークロード行から使えるが、
状況ドロップダウンには「未完了のみ」の選択肢を出していない（URL から来たときだけ解除チップで
可視化する）。常設の切替 UI にするかは、Lite / Pro の絞り込み UI をまとめて見直す差分で決める
（§6「変更は最小スコープに保つ」）。
