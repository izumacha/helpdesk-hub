...
> 本ドキュメントは **戦略・要件レベルのピボット計画書** です。実装タスクは確定次第 `issue-backlog.md` に切り出します（初期開発時の Issue ドラフト集は `archive/github-issues.md` に保管）。
...
- 技術スタック（Next.js 16 / Prisma / PostgreSQL / Server Actions）はそのまま流用可能。**スキーマと UI の簡素化、外部チャネル取り込み、料金体系・オンボーディング刷新** が主な作業。
...
#### 4.28 フォローアップ（2026-09-09）: ダッシュボードの査定で見つかった「事後報告・凍結・行き止まり」の解消

ダッシュボードを世界のヘルプデスク製品のベストプラクティス（意思決定支援・SLA 事前警告・
数字からの drill-down）と突き合わせて厳しく査定し、見つかったギャップをまとめて修正した。

- **Lite タイルの正本仕様準拠**: §3.1 は 2 枚目のタイルを **「期限切れ・今日まで」** と定義して
  いるが、実装は厳密な期限超過（`tab=overdue`）だけを数えており「今日中に対応すべきもの」が
  見えていなかった。タイルを仕様どおり「期限超過 + 今日 (JST) が期限」の件数に改め、ラベルも
  「期限切れ・今日まで」にした。一覧タブ「期限切れ」（Phase 1 の仕様）は従来どおり厳密な超過の
  ままで、タイルは新設の `?due=today` で同一条件の一覧へ遷移する。
- **期限絞り込み `?due=` の新設**（`src/features/tickets/due-filter.ts`）: `soon`（期限間近 =
  SLA 警告帯。`getSlaState` の 'warning' と同じ範囲）と `today`（期限切れ・今日まで）を、
  タブ（`tab-filter.ts`）と同じ「単一の真実の源」パターンで定義し、ダッシュボードの件数と
  drill-down 先の一覧・CSV エクスポートが必ず同じ条件を共有するようにした。タブと違い常設の
  切替 UI が無い絞り込みのため、適用中は一覧に「絞り込み中」チップ（ラベル + 解除リンク）を
  表示し、見えない絞り込みを作らない。
- **SLA の事前警告**: Pro ダッシュボードの「SLA 超過」は超過してから数える事後の指標だけで、
  しかもページ内で唯一クリックできない数字だった。セクションを「対応期限（SLA）」に改め、
  **期限超過**（`tab=overdue` へ drill-down）と **期限間近**（警告帯。`?due=soon` へ
  drill-down）の 2 タイルを並置した。SLA 運用の定石（超過前のしきい値警告 → 事後対応、の
  順で見せる）に合わせた変更で、警告帯の幅はリマインダー通知（issue-backlog #20）と同じ
  `DEFAULT_WARNING_THRESHOLD_MS` を共有する。
- **品質指標の「凍結」解消と負荷対策**: 「対応品質」3 指標は `qualityMetrics` の `since` を
  渡しておらず全期間の累積平均だったため、データが蓄積するほど数字が動かなくなっていた
  （改善も悪化も画面に現れない）。**直近 30 日**（`QUALITY_METRICS_WINDOW_DAYS`、
  `src/lib/dashboard-metrics.ts`）の移動窓に変更し、期間を見出しに明示した。あわせて
  ページ表示のたびに走っていた 3 本の重い生 SQL 集計を `unstable_cache`（60 秒。未読通知数と
  同じ方針）でラップし、Suspense 境界（+ `dashboard/loading.tsx` のスケルトン）へ分離して
  ページ本体の初期表示をブロックしないようにした。
- **配色の一元化と識別性**: SLA の配色が `sla.ts`（yellow/red）とダッシュボード（rose 直書き）の
  2 系統に分裂していたため、アプリのパレット（amber/rose）へ統一し `SLA_COLORS` /
  `SLA_TILE_COLORS` に集約した（§6）。`STATUS_COLORS` はキー型を `Record<TicketStatus, string>`
  にして定義漏れをコンパイル時検出できるようにし、灰色同士で見分けづらかった「新規」を
  violet に変更した（「クローズ」との識別性。§7）。
- **a11y の底上げ**（§7）: ステータスカード・SLA タイル・Lite タイルのリンクに「◯◯ N 件の
  一覧を見る」のアクセシブルネームを付与、ワークロード表の「一覧を見る」を行ごとに一意化、
  空の操作列見出しに `sr-only` の列名、通知ベルの未読件数を読み上げに伝達、
  `prefers-reduced-motion` の全体対応（`globals.css`）、モバイルドロワーの Esc 閉じ +
  フォーカストラップ + フォーカス復元（`Sidebar.tsx`）、指標の「—」に「データ不足」の
  読み上げ、セクション見出しの `aria-labelledby` 関連づけ。
- **その他**: 依頼者が Pro ダッシュボードを開いたときの説明文が「各担当者の負荷を…」のままで
  実際には表示されない内容を約束していたため、ロール別の文言に分けた。ワークロード表に
  最大値比の負荷バー（単色・実数値はセルが持つ装飾）と横スクロールラッパを追加した。
- 回帰テスト: `tests/ticket-due-filter.test.ts`（`parseDueParam` / `applyDueFilter` の境界値、
  JST の日付境界を含む）。
- README のヒーロースクショ（`docs/screenshots/dashboard.png`）は 3 世代前の画面
  （クローズカード・対応品質・拠点フィルタ導入前）のまま放置されていたため、§15 に従い
  本 PR で再撮影した（シードデータのみ・Pro モード・幅 1280px。Playwright + ローカル
  PostgreSQL で撮影）。
- 未対応で残る宿題（意図的）: ステータス別件数の前週比などの**トレンド表示**と Pro
  ダッシュボードの E2E は本フォローアップに含めない（トレンドは表示情報量とのトレードオフを
  §3.1 の「簡素化」方針と突き合わせて別途判断する）。

#### 4.28.1 追補（2026-09-10）: 品質指標の窓の掛け方の是正（レビュー指摘）

上の「品質指標の『凍結』解消」は `qualityMetrics` に `since` を渡すことで直したが、**その
`since` は起票日時（`createdAt`）に掛かる**契約だったため、直したつもりの欠陥が形を変えて
残っていた。窓より長くかかったチケットは**解決した日にはもう窓の外**にいて平均に一度も
入らないため、

- 平均解決時間が窓の長さ（30 日）で**頭打ち**になる、
- 遅い案件が増えるほど「速い分だけ」の平均に寄って数字が**下がる**（= 悪化を改善として表示する）、
- 60 日前に起票され今日再オープンされたチケットは再オープン率の分子にも分母にも現れない、

という状態だった。**窓を各指標の「出来事が起きた時刻」へ移した**（初回応答は
`firstRespondedAt`、解決は `resolvedAt`、再オープンは履歴行の発生時刻）。

あわせて再オープン率の分母を「全チケット」から**「窓の中で対応を終えたチケット」**
（窓内で解決した ∪ 窓内で差し戻された）に改めた。和集合にしているのは、差し戻し時に
`update-ticket.ts` が `resolvedAt` をクリアするため解決分だけでは分子が分母を上回りうる
一方、履歴だけでは CSV 取り込みで `resolvedAt` を直接設定したチケットが分母から抜けるため。
画面の分母ラベルも「全 N 件中」→「対応を終えた N 件中」に改めた（未対応まで含む数字だと
誤読されるため）。

- 回帰テスト（共有契約テスト・実 PostgreSQL で検証済み）:
  「窓は起票日時ではなく対応を終えた時刻に掛かる」（窓より前に起票し窓内で解決した長期案件が
  平均に入ることを固定。**起票日時で切る実装ではこのテストが落ちる**ことを実測で確認）と、
  「差し戻しで `resolvedAt` が消えても分母に残り、率が 1 を超えない」の 2 件。
- 同じレビューで、ワークロード行のリンクの読み上げ名を見えている文字列（「一覧を見る」）と
  一致させ（WCAG 2.5.3 Label in Name。音声操作でリンクを起動できなくなるため）、Lite タイルの
  rose 直書きを `SLA_TILE_COLORS` 参照へ、ページャと絞り込みチップで重複していた URL 組み立てを
  共通ヘルパーへ、モバイルドロワーに `role="dialog"` / `aria-modal="true"`（フォーカストラップ
  だけではスクリーンリーダーの仮想カーソルが背面へ抜けるため）を追加した。
- `docs/screenshots/tickets-list.png` は「新規」バッジの配色変更が写る画面のため §15 に従い
  再撮影した（`ticket-detail.png` はステータスが「未対応」で新配色が写らないため据え置き）。

#### 4.28.2 追補（2026-09-10・2 巡目レビュー）: 再オープン率クエリの計算量と、モーダルの作法

- **相関サブクエリ → LEFT JOIN + 集約**: 4.28.1 で書いた再オープン率の `EXISTS` 版は、実 DB の
  `EXPLAIN` で `SubPlan -> Seq Scan on "TicketHistory"` として残り、**生き残ったチケット 1 行ごとに
  履歴を走査する O(チケット数 × 履歴数)** になっていた（実測 cost 44.18）。LEFT JOIN 1 回 +
  `GROUP BY` に書き換えて履歴を 1 度読むだけにした（同 25.23 → 下記インデックス追加後は 9.30）。
- **`TicketHistory` に `@@index([ticketId, createdAt])` を追加**: PostgreSQL では Prisma が外部キー列に
  インデックスを自動生成しないため、この表には主キーしか無く `ticketId` の絞り込みが常に Seq Scan
  だった（§8）。**この索引を何のために張っているかの説明は `prisma/schema.prisma` の
  `TicketHistory` モデルのコメントが唯一の正本**とし、ここには写さない（3 巡目・4 巡目のレビューで
  実際に、根拠を 3 箇所へ書き写した結果 1 つが古くなる事故を起こしたため）。
  **運用注意**: Prisma の `CREATE INDEX` は `CONCURRENTLY` を付けないため、作成中は当該テーブルへの
  書き込みがブロックされる（マイグレーション本文にも同じ注意を記載）。
- **モーダルの dismiss コントロール**: 4.28.1 で足した `aria-modal="true"` は「このダイアログの外は
  無いものとして扱う」指示なので、Header にあるハンバーガー（それまで唯一の閉じる操作）も支援技術から
  見えなくなる。**Esc キーを持たないタッチ端末のスクリーンリーダー利用者（iOS VoiceOver / TalkBack）が
  メニューから出られなくなる**ため、ドロワー内に md 未満だけ表示する閉じるボタンを追加した
  （ラベルは Header のハンバーガーの開状態ラベルと重複しないものにする。実際の文言は実装が正本）。
  あわせて要素を `<aside>` から `<div>` に変えた（ARIA in HTML は aside に `dialog` ロールを許さず、
  axe の `aria-allowed-role` で新規違反になるため。閉じているときは `role="complementary"` を明示して
  従来と等価に保つ）。
- **効かない配色値の削除と hover の一元化**: `SLA_TILE_COLORS` が持っていた `bg-amber-50/40` /
  `bg-rose-50/30` は、タイル側の基底クラスの `bg-white` が生成 CSS で後に出力されるため**一度も
  描画されていなかった**（ビルド済み CSS の出現位置で実測）。効かない値を一元管理の参照元に残すと
  「変えたのに反映されない」調査を生むので削除し、代わりに画面側が直書きしていた hover 時のリング
  強調を定数側へ移した。
- **分母ラベルの再是正**: 「対応を終えた N 件中」は、分母に含まれる「窓内で差し戻されて**いま未対応に
  戻っている**チケット」を説明できていなかったため「対応が一区切りついた N 件中」に改めた。
- `docs/screenshots/dashboard.png` はこの分母ラベルと SLA タイルの配色が写るため再撮影した。

#### 4.28.3 追補（2026-09-10・3〜4 巡目レビュー）: 閉じたドロワーのタブ順と、写しを増やさない

- **閉じたドロワーをタブ順から外す**: モバイルドロワーは `-translate-x-full` で画面外へずらすだけで、
  `display:none` でも `inert` でもなかった。そのため md 未満でドロワーを閉じていても中のナビリンクが
  タブ順・読み上げ順に残り、**「画面のどこにも見えないリンクにスキップリンクの次で到達し、
  Enter を押すと実際に遷移する」**状態だった（§7 キーボード操作）。閉状態に `invisible`
  （`visibility:hidden`）を足し、md 以上は `md:visible` で常設サイドバーとして戻す。
  3 巡目では新設した閉じるボタン 1 個だけを条件描画にして塞いだが、症状としては
  「押しても無反応なボタン」より「実際に遷移してしまうリンク」のほうが重く、
  要素ごとの特別扱いではなくドロワーごと外すのが根治だった（条件描画は取り下げた。
  要素ごとに出し分けると「タブ順を守っているのはこの条件式だ」と読めてしまうため）。
- **モバイル幅の E2E を新設**（`e2e/mobile-nav.spec.ts`）: このドロワーの a11y は 3 巡続けて
  回帰を出したが、いずれも **lint / typecheck / unit / contract がすべて緑のまま**通っていた。
  既定の Playwright プロジェクトは Desktop Chrome（1280px = md 以上）で、その幅では
  `md:visible` / `md:relative` が仕組みごと覆い隠すため、モバイル幅で見るスペックが 1 本も
  無いと機構全体が無検証だったのが原因。375px 幅で「閉じているあいだナビリンクは
  ロールから引けない（＝フォーカスも読み上げも到達しない）」「開くとフォーカスが
  ドロワー内へ移る」「Tab はドロワー内で循環し背面へ抜けない」「ドロワー内の閉じるボタンで
  閉じられる」「Esc で閉じ、フォーカスがハンバーガーへ戻る」の 5 件を固定した。
...
- §4.28.3 で閉状態に `invisible` を足したが、同要素の `transition-all` は
  ...
  `visible → hidden` の遷移中の計算値は `visible` のままで、`hidden` になるのは終端。
  つまり**閉じてから 200ms のあいだ、ドロワーの中身はフォーカス可能・読み上げ可能なまま
  画面外を滑っていく**（フォーカストラップは既に解除済みなので、その隙に Shift+Tab で
  見えないリンクへ入れてしまう）。§4.28.3 の対策には 200ms の穴が残っていた。
  実際に動かしたいのは位置・幅・影だけなので `transition-[translate,width,box-shadow]` と
  明示列挙し、visibility は即座に切り替わるようにした（生成 CSS で
  `transition-property:translate,width,box-shadow` を確認）。
  **列挙するのは `translate` であって `transform` ではない**（7 巡目レビュー指摘）:
  ...
  **なおこの 200ms の窓は E2E では検出できない**（Playwright の expect は自動再試行するため、
  200ms 後に成立すれば通ってしまう）。CSS の遷移対象を広げるときはこの点に注意すること。
- E2E のハイドレーション競合を回避: `page.goto` は `load` で解決する一方 `MobileNavToggle` は
  SSR されるため、ハイドレーション前でもボタンは存在して操作可能判定を通る。その隙の
  クリックは onClick が未接続で「押せたのに何も起きない」になり、Playwright は成功した
  クリックを再試行しないので後続がタイムアウトする（CI の `retries: 2` が flake として
  覆い隠すため、検出網としてはむしろ有害）。
  当初はクリック前に `aria-expanded` の値を待つ形にしたが、**この属性はサーバ側の初期 HTML にも
  同じ値で出る**ためハイドレーションの前後を区別できず、ガードとして機能していなかった
  （7 巡目レビュー指摘）。**クリックと「開いたか」の確認を 1 組にして `toPass` で再試行する**
  形へ改め、合図の有無に依存しないようにした。共通ログインヘルパー（`e2e/login.ts`）も同じ問題を
  抱えていた（フォームは `action` を持たないので、ハイドレーション前の送信はネイティブ GET になり
  入力値が URL に載る）ため、`goto` からやり直す形で丸ごと再試行するようにした。
- E2E のログインヘルパーを `e2e/login.ts` へ切り出した（同じ関数が 10 スペックへ書き写されて
  いた。§6 DRY）。**既存スペックの移行は別 PR**（無関係な 10 ファイルの書き換えを
  この差分に混ぜないため）。新しいスペックはこのヘルパーを使う。
- `schema.prisma` に書いていた「20 万行規模の実測」への参照を削除。その測定はレビュー時の
  指摘に含まれていただけでリポジトリに記録が無く、参照先（§4.28.2）を辿っても
  シード規模の数値しか無い宙ぶらりんのポインタになっていた。規模が上がるほど
  hash / merge join が選ばれやすい構造的な理由だけを残し、断定はしない形に改めた。
- **索引の根拠の写しを 3 → 1 箇所へ**: 索引を足した理由を `schema.prisma` / `migration.sql` /
  本計画書の 3 箇所に書いた結果、3 巡目で根拠を是正したときに計画書だけが古い記述
  （「2 経路のため」）で取り残された。**説明は `schema.prisma` の 1 箇所を正本**とし、
  マイグレーション本文と本計画書はそこを参照するだけにした。
  あわせて索引の効き方の記述を、断定形から実測に即した条件付きへ改めた
  （内容は `schema.prisma` 側のコメントを参照。ここに再掲すると写しが 2 つに戻る）。
- **適用済みマイグレーションを後から編集しない**: 3 巡目の是正でマイグレーション本文の
  コメントだけを書き換えたが、Prisma はファイル全文のチェックサムを `_prisma_migrations` に
  記録するため、**先に適用済みの DB では次回の migrate が「適用後に変更された」で停止する**
  （CI は毎回まっさらな DB を切るので緑のまま＝気づけるのは手元だけ）。本 PR のマイグレーションは
  未マージなので影響範囲はこのブランチを手元に取った環境に限られるが、本文からは設計根拠を外して
  運用注意だけを残し、以後は触らない方針にした。**このブランチを本コミットより前の任意の時点で
  適用済みの環境は**（本文は 3 回変わっている）、DB を作り直すか `prisma migrate resolve` で
  記録し直すこと。

#### 4.28.5 追補（2026-09-10・9〜10 巡目レビュー）: E2E の再試行が「あるのに効かない」形

§4.28.4 で入れた再試行（`toPass` でクリックごとやり直す形）は、**枠の取り方しだいで
黙って無効化される**。9〜10 巡目はその周辺だけを扱った（本体コードの欠陥は出ていない）。

- **ログインの再試行が前のセッションを再利用していた**（9 巡目）。当初は「`/login` へ行って
  退避させられたら＝既にログイン済みなので成功」と判定していたが、これだと**別のユーザーで
  ログインし直したいときに前のセッションが残ったまま素通り**する。ログインしたつもりのない
  権限でテストが進むので、RBAC・テナント分離のスペックが偽の緑になる（最も危険な壊れ方で、
  しかも `e2e/login.ts` は「新しいスペックはこれを使うこと」と案内している）。
  試行のたびに `context.clearCookies()` からやり直す形へ改め、冪等性も同時に満たすようにした。
  さらに 10 巡目で、クッキーの削除と直前の試行が残した遷移が競合してセッションが生き残った
  場合に備え、**入力の前にログインフォームが実際に出たことを確かめる**（出なければその試行を
  失敗させ、次の試行がもう一度クッキーを捨てる）。以前は入力欄を待ち続けてハングしていた。
- **再試行の総枠がテストの制限時間と同じだと、再試行は 1 度も走らない**（8・10 巡目）。
  `LOGIN_RETRY_BUDGET_MS` は当初 30 秒で、これは **Playwright の既定のテスト制限時間と同値**。
  `playwright.config.ts` に `timeout` を置いていない以上、制限時間を上書きしない呼び出し側では
  「枠 < 制限時間」という前提が既定で破れており、2 回目が始まる前にテストごと打ち切られて
  「Test timeout」だけが残る。枠を既定より小さくして、上書きしない呼び出し側でも
  2 回目が必ず始まるようにした（前提を各スペックの記憶に頼らせない）。
- **枠の内側にも上限が要る**（10 巡目）。`actUntil` が持つのは全体の締切だけなので、
  内側に既定の上限が大きい操作（`page.goto` の既定は 30 秒）を素で置くと、1 回目の試行だけで
  枠を使い切って再試行が走らない。1 回の試行に掛かる上限の合計が枠の半分以下に収まるよう、
  遷移・フォーム表示・着地の 3 つに個別の上限を与えた。**期待される失敗（ハイドレーション前の
  クリックが効かない）では着地の待機だけが枠を使う**ので、枠はその 2 回分＋余白で足りる。
- **1 つの定数を 2 つの用途で共有しない**（10 巡目）。ログインの着地待ちとドロワー表示待ちが
  同じ定数を共有していたため、片方の都合で値を動かすともう片方が黙って変わっていた
  （実際 8 秒 → 5 秒の縮小が理由の説明なくログイン側へ波及していた）。用途ごとに名前を分けた。
- `test.describe.configure` の制限時間は使う枠の合計から導いているが、**この式は
  「1 テストにつき login 1 回・openDrawer 1 回」を前提にしている**。再試行を伴う操作を足す
  ときはその枠もここへ足すこと（足さないと余裕が削られて先に打ち切られる）。
- **Playwright の操作には既定の上限が無い**（11 巡目）。`actionTimeout` の既定は 0＝無制限なので、
  `fill` / `click` / `getAttribute` に上限を渡さないと、1 回の試行が枠もテストの制限時間も超えて
  待ち続ける。上の 2 点を直しても、上限を渡していない操作が 1 つあれば同じ「再試行が走らない」
  状態に戻る。全ての待ちうる操作に上限を渡したうえで、**「1 回の試行の上限の合計 ≦ 枠の半分」を
  読み込み時に確かめる関数**（`assertAttemptFitsBudget`）を置いた。書いてあるだけの前提は
  実際に破られた（この規約を書いた当の `login.ts` が破っていた）ので、条件そのものをコードにする。
- **md 以上へ広げてドロワーが閉じると、フォーカスが `<body>` へ落ちていた**（11 巡目）。
  開いた直後にフォーカスを移す先はドロワー内の先頭要素＝モバイル専用の「ナビゲーションを閉じる」
  ボタンで、これは `md:hidden`。画面を広げるとこの要素も、復元先として覚えていたハンバーガーも
  同時に `display:none` になるため、`focus()` は no-op になり activeElement が `<body>` へ落ちる。
  キーボード利用者は文書の先頭から Tab をやり直すことになる。復元先がまだ描画されているときだけ
  戻し、そうでなければ**常設サイドバーの中の最初のフォーカス可能要素**へ移して居場所を保つ。
  E2E を 1 件追加（旧実装では落ちることを実測）。
- **JS のブレークポイント判定を px で書かない**（12 巡目）。Tailwind v4 の `md:` は
  `@media (min-width:48rem)` を出力する（生成 CSS で実測: 48rem が 1 件、768px は 0 件）。
  `Sidebar` のフォーカストラップだけ `matchMedia('(min-width: 768px)')` で判定していたため、
  既定の文字サイズを 16px から変えている利用者（a11y の設定としてよくある）で CSS と JS の
  境界がずれる。20px なら CSS の境界は 960px なので、幅 800px では見た目はドロワーなのに
  判定は「md 以上」と答えて早期 return し、**Esc もフォーカストラップも初期フォーカス移動も
  効かない**まま `aria-modal="true"` だけが効く（背面が支援技術から隠れているのに Tab で
  そこへ抜けられる）状態になっていた。`48rem` に揃えた。
- **空状態の文言は集計方法を変えたら一緒に直す**（12 巡目）。「対応済みのチケットが蓄積されると
  指標が表示されます」は全期間の累積平均だった頃の文言で、直近 N 日の移動窓になった今は
  「1 年で 500 件解決していても直近 N 日に完了が無ければ出る」ので事実と食い違う。
  窓を明示した文言（日数は定数から組み立て）へ直した。
- **一覧 URL の組み立てを 1 か所へ**（12 巡目）。「今のクエリを複製し、差し替え／取り除き、
  page を先頭へ戻し、空なら "?" を付けない」という同じ規則が 4 箇所（ページャ・期限チップ・
  `TicketTabs`・`TicketFilters`）に書かれていた。§6 の「2〜3 箇所目で共通化する」に達したので
  `features/tickets/tickets-href.ts` の純粋関数へ寄せ、単体テストで規則を固定した。
  **「どのキーを消すか」の方針は呼び出し側に残す**（タブ切替は `due` を消す／絞り込みフォームは
  残す、と意図的に違うため）。共通化するのは組み立ての手順だけ。
- **見送り（記録）**: 期限系フィルタ（`overdue` / `dueSoon` / `dueUntil`）を 1 つの範囲型
  （`dueWithin`）へ一般化する案。同じ「期限の範囲＋未解決」の規則が port とアダプタ 2 つに
  分かれて書かれているのは事実だが、これは Port の契約そのものの作り直しで、影響範囲が
  一覧・エクスポート・ダッシュボードのタイル件数に同時に及ぶ。**次に期限系の絞り込みを
  足すときに、その差分の中で一般化する**（今の 3 つは共有の契約テストが両アダプタで
  固定しているので、その時点でも安全に動かせる）。
- **タイルの件数も一覧と同じフィルタから導く**（13 巡目）。期限系フィルタの条件は
  `buildWhere.addDueCondition` / `matchesFilter` に集約したのに、`dashboardStats` の
  SLA 超過件数だけが同じ条件を自前で書いていた。本 PR でこのタイルは
  「押すと `?tab=overdue` の一覧へ飛ぶ」形になったので、片方だけ直すと
  **「N 件と出ているタイルを開いたら M 件」**になり、しかも一覧側の契約テストは緑のまま。
  両アダプタとも共有の判定を通す形に直した。
- **URL の組み立ての 5 つ目を残さない**（13 巡目）。`dashboard-links.ts` だけ文字列連結で
  `/tickets?...` を作っており、値のエスケープも page のリセットも掛かっていなかった。
  `buildTicketsHref` へ委譲した。
- **`motion-safe:` は重ねない**（13 巡目）。`globals.css` の `prefers-reduced-motion` 指定が
  `*` に対して `animation-duration` を上書きするため既に止まっており、重ねても挙動は変わらず
  「どちらが効いているのか」が読めなくなるだけ（`Sidebar.tsx` の transition と同じ判断）。
  スケルトン 7 箇所から外し、理由をコメントに残した。
- **見送り（記録）: `text-slate-400` のコントラスト不足**。白地に `#94a3b8` は 2.56:1 で、
  §7 が求める AA（通常文 4.5:1 / 大きな文字 3:1）を満たさない。ただしこれは本 PR が
  持ち込んだものではなく **`src/` 全体で 61 箇所**使われている既存の「補助テキスト」トークンで、
  ダッシュボードだけ `slate-500`（4.76:1）へ変えると同じ画面に 2 つの濃さが混在する。
  **トークンの置き換えは専用の PR で全画面まとめて行う**（スクリーンショットの撮り直しも
  そこで一度に済ませる）。§6 の「変更は最小スコープに保つ」に従いここでは触らない。
- **見送り（記録）: タブの「期限切れ」と SLA の「期限超過」の呼び分け**。同じ条件に 2 つの
  呼び名があるのは事実だが、タブは Lite / Pro 共通の UI で、`SLA_LABELS` は Pro の SLA 語彙。
  タブを `SLA_LABELS.overdue` から導くと Pro の用語が Lite 画面へ流れ込み、正本 §3.1 の
  **用語簡素化**（Lite はやさしい言葉を使う）と衝突する。呼称の統一は用語設計の判断なので、
  Lite/Pro の語彙表を見直す差分でまとめて決める。

#### 4.28.6 追補（2026-09-15・事後レビュー）: 「件数と drill-down の一致」の残り 1 箇所と、絞り込みの上書き

§4.28 は SLA 系タイルについて「タイルの数字と遷移先の一覧の件数が一致すること」を実現したが、
**同じ画面のワークロード行だけが取り残されていた**。合わせて、その修正の前提になる
データ層の穴が 1 つ見つかった。

- **`status` と `statusIn` が上書きし合っていた**（最も影響が大きい）。Prisma アダプタの
  `buildWhere` は `where.status` へ順に代入しており、`statusIn` が `status` を黙って消して
  いた。メモリアダプタは 2 つを別々に判定するので、**両アダプタの答えが食い違う**状態。
  `?tab=mine&status=Resolved`（タブが `statusIn`、状況ドロップダウンが `status` を立てる）は
  画面から 1 クリックで作れるため、「解決済み」を選んでいるのに未対応チケットが並んでいた。
  CSV エクスポートも同じ `buildTicketListFilter` を通るので書き出す範囲まで一緒にずれる。
  Prisma の enum フィルタは `equals` と `in` を同じオブジェクトに置くと AND で評価されるので、
  期限系フィルタ（`addDueCondition`）と同じく 1 つのフィルタへ積み上げる形へ直した。
- **ワークロード行の drill-down が件数と一致していなかった**。表の件数は
  `excludeStatusesForWorkload` で終息ステータスを除いた数なのに、リンク先は
  `?assigneeId=...` だけで状態条件を持たず、完了済みまで含む全件が並んでいた。
  リンクの読み上げ名が「〇〇 の未完了 N 件の一覧を見る」と件数の意味を明言しているぶん、
  a11y の面でもそのまま害になる。「未完了」を URL で表現する手段が無かったのが原因なので、
  タブ・期限と同じ単一の真実の源パターンで `?open=1`（`open-filter.ts`）を新設した。
- **空の `statusIn` は「絞り込みなし」ではなく「0 件」**。`applyOpenFilter` は既存の
  `statusIn` と積集合を取る（上書きすると `?tab=mine&open=1` で片方が黙って消えるため）ので、
  空配列が作られうる。空を「指定なし」と読むと、条件が厳しすぎて 0 件のはずの絞り込みが
  **全件表示に化ける**（fail-open）。両アダプタで 0 件として扱う契約に揃え、共有契約テストで固定した。
- **終息ステータスの写しを 1 箇所へ**（当初の差分では memory アダプタの再オープン率判定に
  1 箇所取り残しがあり、事後レビューで指摘されて解消した。**件数は書かない** — 直すたびに
  この数字だけが古くなる）。`'Resolved' / 'Closed'` の組が Prisma アダプタ・
  メモリアダプタ・ダッシュボードに散らばっており、終息ステータスを 1 つ足しても型エラーに
  ならないまま片方だけが取り残される形だった（どの写しも fail-open）。
  `domain/ticket-status.ts` に `COMPLETED_STATUSES` / `UNRESOLVED_STATUSES` を置き、
  **全ステータス一覧は遷移表 `ALLOWED_TRANSITIONS` のキーから導出**して追随を型で強制する。
- **「必ず 0 件」の組み合わせを作る入口が 2 つあり、方針が食い違っていた**。タブ切替は
  §4.28 で `?due=` を落とすようにしたのに、状況ドロップダウンは落としておらず、
  `?due=soon` の一覧で状況に「解決済み」を選ぶと同じ状態へ 1 クリックで落ちていた。
  **片側だけ塞いでも意味が無い**（`?status=Resolved` の一覧で「自分の未対応」タブを押せば
  同じ行き止まりに落ちる）ので、`build-filter.ts` に述語 `tabExcludesStatus` を 1 つ置き、
  状況ドロップダウン（`filtersClearedByStatusChange`）とタブ（`filtersClearedByTabChange`）の
  **両方がそれを共有する**形にした。述語はタブの条件を書き写さず `applyTabFilter` を実際に
  通した結果から導くので、タブの意味を変えてもここだけが古い前提で残ることはない。
  なお行き止まりは終息ステータスに限らない（`tab=mine` は `New` / `WaitingForUser` /
  `Escalated` も除く）ため、判定は「終息かどうか」ではなく「そのタブがその状態を除くか」で行う。
- **窓を移した先の列に索引が無かった**。§4.28.1 で品質指標の窓を `createdAt` から
  `firstRespondedAt` / `resolvedAt` へ移したが、索引は `TicketHistory` 側にしか足して
  おらず、キャッシュミスのたびにテナントの全チケットを走査していた。
  索引の根拠は §4.28.4 の方針どおり `schema.prisma` の 1 箇所を正本とし、ここには再掲しない。
- **`(param IS NULL OR col >= param)` は索引が使える形ではない**（事後レビュー指摘）。
  窓の条件をこの形で書くと、プランナが計画時にパラメータの NULL 判定を決められないため、
  汎用計画（generic plan）が選ばれた瞬間に索引が無視される。索引を足した意味が
  「たまたまカスタム計画が選ばれること」に依存していた。窓を無効にしたいときは
  PostgreSQL の `-infinity` を渡し、条件そのものは常に `col >= $n::timestamptz` の形で
  書く形へ改めた（実測値は `schema.prisma` のコメントが正本）。
- **`Prisma.sql` の断片を入れ子にして SQL を出し分けない**（E2E が捕まえた回帰）。
  上の是正で最初に採ったのが断片の入れ子で、**ソースから直接動かす vitest では通るのに
  `next build` した本番バンドルでは入れ子の `Sql` がバインドパラメータとして扱われ**、
  `syntax error at or near "$2"` で Pro ダッシュボードが丸ごと 500 になった。
  型チェック・lint・ユニット・契約テスト（実 DB を使うものを含む）がすべて緑のまま、
  **E2E だけが落ちる**壊れ方をする。生 SQL を条件で組み替えたくなったら、
  断片の入れ子ではなく「すべての行に当てはまる番兵の値」を渡して条件を固定する。
  この経緯は `ticket-repository.prisma.ts` の当該クエリのコメントが正本。

**残っている課題（記録）**:

- **再オープン率クエリ（3 本目）の窓は索引で絞れない**。窓の条件が結合結果
  （`th.id IS NOT NULL`）との OR の中にあり、結合より前に評価できないため索引条件にならない。
  実測では `Ticket` の全走査になる（数値は `schema.prisma` のコメントが正本）。解消には分母を
  「窓内で解決した id」と「窓内で差し戻された id」の UNION として組み直す必要があり、
  §4.28.2 で一度チューニングした形を作り替えることになる。**本 PR の目的（窓を移した先に
  索引が無かったことの是正）とは別の変更**なので、専用の差分で扱う。
- **`?open=1` に常設の切替 UI が無い**。一覧・CSV エクスポート・ワークロード行からは使えるが、
  状況ドロップダウンには「未完了のみ」の選択肢を出していない（URL から来たときだけ解除チップで
