...
- **`TicketHistory` に `@@index([ticketId, createdAt])` を追加**: PostgreSQL では Prisma が外部キー列に
  ...
  だった（§8）。引くのは上記の再オープン率の JOIN と、チケット詳細の変更履歴（`ticketId` で絞り
  `createdAt` 降順で上限件数）の 2 経路で、後者に合わせて第 2 キーを `createdAt` にしている。
...
- **モーダルの dismiss コントロール**: 4.28.1 で足した `aria-modal="true"` は「このダイアログの外は
  ...
  メニューから出られなくなる**ため、ドロワー内に md 未満だけ表示する「メニューを閉じる」ボタンを追加した。
