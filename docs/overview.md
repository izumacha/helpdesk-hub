...
| 機能 | 状態 | 備考 |
| --- | --- | --- |
| ダッシュボード | ✅ 実装済 | ステータス別件数、SLA 期限超過・期限間近件数（該当一覧へのドリルダウン付き）、担当者別ワークロード、対応品質指標（直近 30 日） |
| 添付ファイル | ✅ 実装済 | チケット・コメントへの画像添付（10MB<!--size:MAX_ATTACHMENT_SIZE_BYTES--> 上限・MIME 検証） |
