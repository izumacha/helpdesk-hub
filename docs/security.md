...
> - 上記およびこの節の**散文**に出てくるサイズ表記 … `tests/doc-body-size-drift.test.ts`。各数値の直後に不可視の HTML コメントで出典の定数名（例: `<!--size:ATTACHMENT_UPLOAD_MAX_BODY_BYTES-->`）を書いておき、定数の現在値と一致することを要求する。**注記の無いサイズ表記を書き足すと落ちる**ので、新しく数値を書くときは出典も併せて書くこと。
...
上限がさらに小さい経路（LINE 取り込み 256KB<!--size:LINE_WEBHOOK_MAX_BODY_BYTES--> / マジックリンクのコールバック 64KB<!--size:MAGIC_LINK_CALLBACK_MAX_BODY_BYTES-->）を前段でも絞りたい場合は、同じ規則（経路の上限 + 余裕）で `location` を足す。足さなければ既定の 2m を継承するだけで、アプリ側の 413 とログは働く。
