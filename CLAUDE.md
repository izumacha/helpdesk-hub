...
整合性は `tests/stripe-api-version-guard.test.ts` が機械的に見張る。見張る対象は 5 つ: (a) SDK の申告するメジャー版が想定どおりか（`EXPECTED_STRIPE_MAJOR_API_VERSION` を書き換えてガードを黙らせようとした場合もここが落ちる）、(a') SDK 内部で `API_VERSION` と `MAJOR_API_VERSION` の申告が整合しているか（上流の生成ミスで両者が食い違うと (a) だけでは通ってしまう。**定数の書き換えを検出する役目は (a) が負っており、こちらではない**）、(b) `apiVersion` の指定がソースに残っているか、(b') 配線された版が SDK の申告値と一致するか、(c) ソースに日付入りリテラルが復活していないか（現在は正しい値でも、次の bump で必ず壊れる潜在的な写しなので落とす）。
...
パーサならコメントはトークンにならず（コメントに日付版を書いても誤検知しない）、判定を**実際の `new Stripe(...)` の引数に固定**できるので、これらはまとめて消える。コメント除去も、その除去を見張る生存確認テストも不要になった（(b) が「呼び出しを特定できたか」まで見るため、対象を見失った場合も落ちる）。同じ理由で (a') の比較も正規表現をやめ `endsWith` にしている（メジャー名に `.` や `(` が入ったとき、誤って通す／例外で落ちる のどちらも避ける）。
