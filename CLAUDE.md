...
整合性は `tests/stripe-api-version-guard.test.ts` が機械的に見張る。見張る対象は 5 つ: (a) SDK の申告するメジャー版が想定どおりか、(a') その想定値が**実際の版の接尾辞と一致する**か（定数を適当な値へ書き換えるだけでガードを無力化できないようにする）、(b) `apiVersion` の指定がソースに残っているか、(b') 配線された版が SDK の申告値と一致するか、(c) ソースに日付入りリテラルが復活していないか（現在は正しい値でも、次の bump で必ず壊れる潜在的な写しなので落とす）。

**(b) と (b') を分けているのは、実行時の値だけを見る検査が「指定の消失」を原理的に検出できないため。** SDK は `version: props.apiVersion || DEFAULT_API_VERSION` と実装されていて、この `DEFAULT_API_VERSION` は `Stripe.API_VERSION` そのもの。つまり `apiVersion` の指定を丸ごと消しても `getApiField('version')` は同じ値を返し続ける（実測で、指定を削除しても全件緑のままだった）。「指定が在ること」はソース側で (b)、「指定の値が正しいこと」は実行時に (b') で見る。**実行時の検査だけに寄せると、SDK の既定値に隠れて静かに fail-open する。**

(b) と (c) はコメントを除いたコード部分を走査する（経緯の説明として日付版がコメントに登場しうるため）。除去は**行頭だけでなく行末コメントも対象**にする（同ファイルは行末コメント形式を実際に使っており、行頭だけ除く実装ではコードが綺麗でも赤くなる誤検知になる）。あわせて、除去が効きすぎて**検査が空振りで緑になる**ことがないよう、走査対象に `Stripe.API_VERSION` が残っていることを先に確かめる（判定とは独立な手がかりで検出網の生存を見張る）。
