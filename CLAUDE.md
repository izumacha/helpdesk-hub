...
- **未完了（後続 PR）**: 全 Server Action / Query での `where.tenantId = session.user.tenantId` 強制、middleware でのスコープ必須化、テナント作成画面。**新規 Server Action を書く際は冒頭で `session.user.tenantId` を取り出して `where` に必ず差し込む**（足し忘れはクロステナント漏洩）。
