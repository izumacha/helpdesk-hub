...
- **(b0) は実行時チェックの守備範囲外。** 2 つ目のクライアントは `getStripeClient()` を通らないので、実行時チェックをいくら強くしても見えない。古いメジャーへピン留めした 2 つ目が黙って入るのを防ぐ。生成箇所の検出は**ファイルごとに `stripe` の束縛名を import 宣言から解決**し、デフォルト import（`new Stripe(...)`）と名前空間 import（`new Ns.default(...)`）の両方を見る（名前を決め打ちすると別名・名前空間の 2 つ目が見えない。どちらも実測で素通りした）。**Stripe クライアントを増やすときはこのテストが落ちるので、増やす側が API バージョンの扱いを決めてから通すこと。**
