...
整合性は `tests/stripe-api-version-guard.test.ts` が機械的に見張る。見張る対象は 6 つ: (a) SDK の申告するメジャー版が想定どおりか（`EXPECTED_STRIPE_MAJOR_API_VERSION` を書き換えてガードを黙らせようとした場合もここが落ちる）、(a') SDK 内部で `API_VERSION` と `MAJOR_API_VERSION` の申告が整合しているか（上流の生成ミスで両者が食い違うと (a) だけでは通ってしまう。**定数の書き換えを検出する役目は (a) が負っており、こちらではない**）、(b0) **`new Stripe(...)` が `src` 全体でちょうど 1 つ**か、(b) その呼び出しの `apiVersion` に名前付き定数が渡されているか、(b') 配線された版が SDK の申告値と一致するか、(c) ソースに日付入りリテラルが復活していないか（現在は正しい値でも、次の bump で必ず壊れる潜在的な写しなので落とす）。
