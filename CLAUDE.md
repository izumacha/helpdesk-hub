...
整合性は `tests/stripe-api-version-guard.test.ts` が機械的に見張る。見張る対象は 6 つ: (a) SDK の申告するメジャー版が想定どおりか、(a') SDK 内部で `API_VERSION` と `MAJOR_API_VERSION` の申告が整合しているか（上流の生成ミスで両者が食い違うと (a) だけでは通ってしまう）、(b0) **`new Stripe(...)` が `src` 全体でちょうど 1 つ**か、(b) その呼び出しの `apiVersion` に名前付き定数が渡されているか、(b') 配線された版が SDK の申告値と一致するか、(c) `src` 全体に日付入りリテラルが復活していないか（現在は正しい値でも、次の bump で必ず壊れる潜在的な写しなので落とす）。

**(a) は改ざん検知ではない。** 定数を新しいメジャー名へ書き換えれば当然また緑になり、それは「移行を済ませた」場合と「確認せず黙らせた」場合とで**まったく同じ編集**なので、テストの側から両者を区別することは原理的にできない。このガードの役目は「メジャーが変わったことを人の目に必ず一度通す」ことであって、その先の判断を強制することではない（`EXPECTED_STRIPE_MAJOR_API_VERSION` が動く差分は、レビューで移行の有無を確認する）。

**(b0) が無いと (b) は「どれか 1 つの呼び出しに `apiVersion` があればよい」検査に退化する。** 囮の 2 つ目を足せば本番のシングルトンから指定が消えても緑のまま通り、別モジュールが古いメジャーへピン留めした 2 つ目のクライアントを足しても気付けない（どちらも実測で素通りを確認）。0 件（検出網が対象を見失った）と 2 件以上の両方を fail-closed で落とす。**Stripe クライアントを増やすときはこのテストが落ちるので、増やす側が API バージョンの扱いをどうするか決めてから通すこと。**

**(b) と (b') を分けているのは、実行時の値だけを見る検査が「指定の消失」を原理的に検出できないため。** SDK は `version: props.apiVersion || DEFAULT_API_VERSION` と実装されていて、この `DEFAULT_API_VERSION` は `Stripe.API_VERSION` そのもの。つまり `apiVersion` の指定を丸ごと消しても `getApiField('version')` は同じ値を返し続ける（実測で、(b) を足す前の 4 件は指定を削除しても全件緑のままだった）。「指定が在ること」はソース側で (b)、「指定の値が正しいこと」は実行時に (b') で見る。**実行時の検査だけに寄せると、SDK の既定値に隠れて静かに fail-open する。** なお (b) は `apiVersion` に**名前付き定数が渡されていること**だけを見て、定数名そのものは固定しない（名前を変えただけのリファクタで赤くしないため。値の正しさは (b') が担う）。

**ソースを見る検査（(b) と (c)）は正規表現ではなく TypeScript のパーサで行う**（`tests/lib/source-module-graph.ts` の `parseSourceFile` / `visitNodes` を再利用。§6 DRY。同ヘッダが「正規表現ではなくパーサを使う」理由を書いているのと同じ判断）。正規表現による検出網は「拾いすぎ」も「取り落とし」も**検査を緩める方向**に効く。実際、初版の正規表現版には次の 3 つの穴があり、いずれも実測で素通りを確認した:

- 行コメント中の `/*` と `*/` が 1 つのブロックコメントとして繋がり、その間の**実コードごと**走査対象から消える → 日付リテラルを直書きしても (c) が緑のまま通る
- `apiVersion: undefined`（＝指定しないのと同義）を「名前付き定数」と誤認する → 指定を無効化しても (b) が通る
- ファイル全体を走査するため、`new Stripe(...)` から `apiVersion` を外しても、無関係な場所に `apiVersion:` が 1 つあれば (b) が通る

パーサならコメントはトークンにならず（コメントに日付版を書いても誤検知しない）、判定を**実際の `new Stripe(...)` の引数に固定**できるので、これらはまとめて消える。コメント除去も、その除去を見張る生存確認テストも不要になった（(b0)/(b) が「呼び出しを特定できたか」まで見るため、対象を見失った場合も落ちる）。同じ理由で (a') の比較も正規表現をやめ `endsWith` にしている（メジャー名に `.` や `(` が入ったとき、誤って通す／例外で落ちる のどちらも避ける）。

なお (c) が探すのは `.<メジャー名>` まで含む形ではなく**日付そのもの**で、テンプレートリテラルの断片（Head / Middle / Tail）まで拾う。`` `2026-07-29.${major}` `` のように分割して書かれた写しは日付部分しか 1 つのトークンに残らず、接尾辞まで要求すると取り落とすため（実測でこの形が素通りしていた）。(b) はショートハンド（`{ apiVersion }`）も受け付け、スプレッドで静的に読めない形は「指定なし」ではなく「不透明」として区別して報告する（診断が実態と食い違わないように）。

**走査は「1 ファイル」ではなく `src` 全体**で、`new Stripe(...)` の検出は**ファイルごとに `stripe` のデフォルト import の束縛名を解決**してから行う（名前を `Stripe` に決め打ちすると `import StripeSdk from 'stripe'` で作った 2 つ目のクライアントが見えない）。(c) を 1 ファイルに絞ると、日付入りの写しを別モジュールへ置いて名前付き定数として import し直すだけで (b)(c) を迂回できる。いずれも実測で素通りを確認して塞いだもの。`src` の走査と生成物の除外は `tests/lib/source-module-graph.ts` の `parseSourceFiles` を使う（`tests/entry-body-limit.test.ts` と共有。写しを持つと片方だけ直したときにもう片方が静かに緩む）。走査結果はモジュール評価時に 1 度だけ求めて全テストで使い回す。

**「渡している値が実際に使えるものか」はテストではなく本番モジュール側の実行時チェックが担保する。** `getStripeClient()` はクライアントを作る前に `assertApiVersionSupported()` で「値が空でない」「想定したメジャーである」ことを確かめ、満たさなければ throw する（§9 fail-closed。同モジュールが `STRIPE_SECRET_KEY` 未設定で起動を失敗させるのと同じ扱い）。**この分担が要点**で、静的検査だけで値の妥当性を保証しようとすると `undefined` / `void 0` / 別名の連鎖…と綴りごとに追いかける終わりのない作業になり、しかも 1 つ漏らすたびに静かな fail-open が生まれる。実行時チェックは綴りに依存しないので、その系統の穴がまとめて閉じる。ソース側の (b) が担うのは**「指定が書かれているか」だけ**——これは SDK の既定値に隠れて実行時からは見えないので、静的にしか確かめられない。あわせて、これで `EXPECTED_STRIPE_MAJOR_API_VERSION` が「テストからしか読まれない飾り」でなくなり、テストを流さずデプロイしても想定外のメジャーで喋り始めることがなくなる。

`apiVersion` の**指定そのものは、現時点では実行時の挙動を変えない**（SDK の既定値が `Stripe.API_VERSION` そのものであるため）。それでも明示し続けるのは防御的な意味で、将来 SDK の既定値が `API_VERSION` と食い違う形に変わったときに送る版が意図から外れるのを防ぐため。この「今は無害でも外れると効いてくる指定」が黙って消えないようにするのが (b0)/(b) の役割にあたる。

なお `STRIPE_API_VERSION` の型注釈（`Stripe.LatestApiVersion`）は**上流の型が緩むことへの防御にはならない** — `LatestApiVersion` も `API_VERSION` も同じ `typeof ApiVersion` なので、上流が素の `string` へ広げると代入の両側が同時に広がって通る。型が緩んだ場合に実際に効くのは (a)（型ではなく実行時の文字列を見る）の方。
