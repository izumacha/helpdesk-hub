...
**走査は「1 ファイル」ではなく `src` 全体**で、`new Stripe(...)` の検出は**ファイルごとに `stripe` のデフォルト import の束縛名を解決**してから行う（名前を `Stripe` に決め打ちすると `import StripeSdk from 'stripe'` で作った 2 つ目のクライアントが見えない）。(c) を 1 ファイルに絞ると、日付入りの写しを別モジュールへ置いて名前付き定数として import し直すだけで (b)(c) を迂回できる。(b) は識別子の**宣言まで辿って** `undefined` への束縛を弾く（`apiVersion: undefined` は直接弾けても、`const V = undefined` を経由すると素通りする）。いずれも実測で素通りを確認して塞いだもの。`src` の走査と生成物の除外は `tests/lib/source-module-graph.ts` の `parseSourceFiles` / `isGeneratedPath` を使う（`tests/entry-body-limit.test.ts` と共有。写しを持つと片方だけ直したときにもう片方が静かに緩む）。走査結果はモジュール評価時に 1 度だけ求めて全テストで使い回す。
