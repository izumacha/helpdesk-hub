...
(b) と (c) はコメントを除いたコード部分を走査する（経緯の説明として日付版がコメントに登場しうるため）。除去は**行頭だけでなく行末コメントも対象**にする（同ファイルは行末コメント形式を実際に使っており、行頭だけ除く実装ではコードが綺麗でも赤くなる誤検知になる）。あわせて、除去が効きすぎて**検査が空振りで緑になる**ことがないよう、検出網の生存確認を**独立した 1 件のテスト**として持つ。目印には `export function getStripeClient` のように**この規約が一切制約しない**ものを使う — `Stripe.API_VERSION` のような「規約が縛っている式」を目印にすると、規約に沿ったリファクタ（分割代入など）で生存確認が誤って落ち、しかも「コメント除去が壊れた」という見当違いのメッセージが出る。生存確認を (b)(c) の中で兼ねないのも同じ理由で、兼ねると本来の違反（リテラル復活など）が「検出網が死んだ」と報告されてしまう。
