...
`tests/stripe-api-version-guard.test.ts` が見張るのは、**実行時チェックでは原理的に見えない静的な性質**に絞る（7 件）: (a) SDK の申告するメジャー版が想定どおりか、(a') SDK 内部で `API_VERSION` と `MAJOR_API_VERSION` の申告が整合しているか、(b0') **`stripe` を実行時に import するファイルが `src` 全体で 1 つ**か、(b0) **Stripe クライアントの生成箇所が `src` 全体でちょうど 1 つ**か、(b) 実行時チェックが**生成箇所と同じ関数の中で、生成より前に**呼ばれているか、(b') 配線された版が SDK の申告値と一致するか、(c) `src` 全体に日付入りリテラルが復活していないか。

- **(b0) は実行時チェックの守備範囲外。** 2 つ目のクライアントは `getStripeClient()` を通らないので、実行時チェックをいくら強くしても見えない。古いメジャーへピン留めした 2 つ目が黙って入るのを防ぐ。生成箇所の検出は**ファイルごとに `stripe` の束縛名を import 宣言から解決**し、デフォルト import（`new Stripe(...)`）・名前空間 import（`new Ns.default(...)`）・名前付き import（`import { Stripe } from 'stripe'`。stripe はクラスを default と名前付きの両方で公開している）のすべてを見る（名前や形を決め打ちすると 2 つ目が見えない。3 形とも実測で素通りした）。さらに (b0') が「SDK がどのファイルへ到達しているか」だけを見て、再エクスポート・`import = require`・動的 import・素の `require()` のような**束縛が別モジュール経由で渡る形**もまとめて捉える（(b0) は import の形を解決してから `new` を探すためそこに弱く、いずれも実測で素通りした）。なお (b0') が数えるのは**この 5 つの読み込み方**であって、「どんな書き方でも必ず捉える」わけではない。**Stripe クライアントを増やすときはこの 2 つが落ちるので、増やす側が API バージョンの扱いを決めてから通すこと。**
- **(b) は「その生成経路に配線されているか」を見る。** 実行時チェックを消すと ESLint は「未使用」と言うが **warning 止まりで CI は緑のまま**通るため、ここで落とす必要がある。ただし「ファイルのどこかに呼び出しがあるか」では足りない — 一度も呼ばれない関数の中へ移すだけで通ってしまい、実行時の担保が黙って外れる（実測で全件緑になった）。生成箇所を囲む関数の中で、生成より前に呼ばれていることまで求める。
