...
describe.skipIf(!hasOpenssl)('POST /api/auth/sso/[tenantId]/acs のリプレイ防止', () => {
  ...
  // /code-review ultra 指摘対応: ユーザー未登録 (sso-no-user) で失敗したアサーションは
  // 「消費済み」として焼却してはいけない。焼却してしまうと、後で管理者がアカウントを作成しても
  // (アサーション自体がまだ有効期限内であっても) 同じアサーションで二度とログインできなくなる。
  it('ユーザー未登録で失敗したアサーションはリプレイ扱いにならず、後でユーザー作成後に成功する', async () => {
    // 対象ユーザーをまだシードしない状態でリクエストする
    store.users.delete('user-1');
    const samlResponse = makeSignedResponse('_assertion-no-user-then-created');

    // 1 回目: ユーザーが存在しないため sso-no-user で拒否される
    const first = await postAcs(samlResponse);
    ...
    // 2 回目: 同じアサーションでも「消費済み」として焼却されていないため、今度は成功する
    const second = await postAcs(samlResponse);
    expect(second.status).toBe(200);
  });
});
