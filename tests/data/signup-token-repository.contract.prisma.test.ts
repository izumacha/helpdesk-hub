// セルフサーブサインアップトークンリポジトリ (Prisma アダプタ) の契約テスト。
// MagicLinkRepository と同じ理由 (consumeValidToken の単回使用保証は PostgreSQL の
// 行ロックの上で実際に成立するかがメモリアダプタでは検証できない) で、実 DB に対する
// 契約テストを用意する。SignupToken は MagicLinkToken と同じく tenantId を持たない
// (発行時点ではまだテナントが存在しない) ため、Tenant/User のシードは不要。
//
...
describe.runIf(SHOULD_RUN)('SignupTokenRepository (prisma adapter)', () => {
  ...
  // 各テスト前に関連テーブルを空にする (SignupToken は tenantId 非依存なので単独で足りる)
  beforeEach(async () => {
    await prisma.$executeRawUnsafe('TRUNCATE TABLE "SignupToken" RESTART IDENTITY CASCADE');
  });
  ...
  it('createで発行したトークンは未消費 (consumedAt=null) でfindByTokenHashから取得できる', async () => {
    ...
    // トークンを 1 件発行する
    const created = await repos.signupTokens.create({
    ...
    const found = await repos.signupTokens.findByTokenHash('hash-1');
  ...
  it('consumeValidTokenは未消費+失効前なら1度だけ成功する', async () => {
    ...
    const now = new Date();
    // 1 回目の消費: 成功して行が返るはず
    const first = await repos.signupTokens.consumeValidToken({ tokenHash: 'h-once', now });
    expect(first?.email).toBe('b@example.com');
    expect(first?.consumedAt).toBeInstanceOf(Date);

    // 2回目は既に消費済みなので null
    const second = await repos.signupTokens.consumeValidToken({ tokenHash: 'h-once', now });
  ...
  it('consumeValidTokenは失効済みならnullを返し消費印も立てない', async () => {
    ...
    // 失効済みトークンの消費を試みる
    const result = await repos.signupTokens.consumeValidToken({
      tokenHash: 'h-expired',
      now: new Date(),
    });
    // 消費に失敗して null が返ること
    ...
    const row = await repos.signupTokens.findByTokenHash('h-expired');
  ...
  it('consumeValidTokenは同時2リクエストでも実DBの行ロックにより成功が1件のみになる', async () => {
    ...
    const [a, b] = await Promise.all([
      repos.signupTokens.consumeValidToken({ tokenHash: 'h-race', now }),
      repos.signupTokens.consumeValidToken({ tokenHash: 'h-race', now }),
    ]);
    // 成功 (null でない) 側を抽出する
  ...
  it('deleteByIdは指定IDを削除し存在しないIDでも例外を投げない', async () => {
    ...
    // 発行したトークンを削除する
    await repos.signupTokens.deleteById(created.id);
    // 削除後は findByTokenHash で見つからないこと
    expect(await repos.signupTokens.findByTokenHash('h-delete')).toBeNull();
    // 存在しない ID でも例外を投げない (P2025 を避けて deleteMany を使う設計の確認)
    await expect(repos.signupTokens.deleteById('no-such-id')).resolves.not.toThrow();
  });
  ...
  it('deleteExpiredは失効済みのみ削除する', async () => {
    ...
    // 期限切れ一括削除を実行する
    const removed = await repos.signupTokens.deleteExpired(now);
    // 削除件数は 1 件のはず (失効済みの 1 件のみ)
    expect(removed).toBe(1);
    // 有効なトークンは残っていること
    expect(await repos.signupTokens.findByTokenHash('valid')).not.toBeNull();
    // 失効済みトークンは削除されていること
    expect(await repos.signupTokens.findByTokenHash('expired-1')).toBeNull();
  });
  ...
  it('countRecentByEmailは同一メール宛のsince以降の件数を返す', async () => {
    ...
    const old = await repos.signupTokens.create({
    ...
    expect(await repos.signupTokens.countRecentByEmail('rate@example.com', since)).toBe(1);
