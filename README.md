...
<p align="center">
  <img src="docs/screenshots/ticket-flow.gif" alt="代表フローのデモ動画。依頼者がチケットを起票し、担当者がステータスを対応中へ変更すると、依頼者に通知が届くまでの流れ" width="900" />
  <br />
  <em>デモ — 起票（依頼者）→ ステータス遷移（担当者）→ 通知（依頼者）までの代表フロー（シードデータ使用）</em>
</p>
...
| カテゴリ         | 機能                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| マルチテナント   | テナント分離（Lite / Pro の 2 モード）、セルフサーブサインアップ、メンバー招待                                                 |
| 認証             | パスワード / マジックリンク / SAML SSO（Enterprise 限定）、ロール別アクセス制御（requester / agent / admin）、認証イベント監査 |
...
未設定だと本文が既定 10MB で黙って切り詰められるため必須の設定）。
余白を足すのは省略できません。枠を最大値ちょうどにすると、入口が「枠を跨ぐチャンク」をまるごと
捨てて閉じるためルート側へは常に上限以下しか届かず、**ルート側の 413 が到達不能になります**
（詳細は `src/lib/entry-body-limit.ts` の `ENTRY_OVER_LIMIT_MARGIN_BYTES`）。この枠は**アプリ全体で 1 つ
...
│   ├── (app)/              # 認証済みレイアウト
│   │   ├── dashboard/      # ダッシュボード
│   │   ├── tickets/        # チケット一覧・詳細・新規・CSV取り込み
│   │   ├── faq/            # FAQ候補管理
│   │   ├── notifications/  # 通知一覧
│   │   ├── audit/          # 監査ログ（admin・Pro/Enterprise 限定）
│   │   ├── quarantine/     # 隔離メール/LINE（admin）
│   │   └── settings/       # テナント設定（admin）
│   ├── api/                # API Routes（SSE 通知・メール/LINE 取り込み・Stripe Webhook 等）
│   ├── login/              # ログイン画面
│   ├── signup/             # セルフサーブサインアップ
│   ├── invite/             # 招待受諾
│   └── help/               # ヘルプセンター
...
├── migrations/
└── seed.ts
scripts/                    # DB バックアップ・リストア
docs/                       # 設計資料
tests/                      # ユニットテスト・Prisma 契約テスト
e2e/                        # E2E テスト
```
