...
| ダッシュボード   | ステータス別件数、SLA期限超過・期限間近件数（該当一覧へのドリルダウン付き）、担当者別ワークロード、対応品質指標（直近の移動窓） |
| FAQ候補          | 解決済み問い合わせのFAQ変換（公開/却下管理）                                                     |
| AI FAQ 自己解決  | 起票前に下書きへ似た公開済み FAQ を AI で照合して提示（引用は FAQ 本文そのまま、Pro/Enterprise 限定・`ANTHROPIC_API_KEY` 設定時） |
