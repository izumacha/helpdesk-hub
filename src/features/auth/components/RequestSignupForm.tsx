...
// サインアップ発行用の Server Action
import { requestSignup } from '@/features/auth/actions/request-signup';

// サインアップ (メール入力) フォーム。送信後は同じカード内で確認メッセージに切り替える
// (request-magic-link と同じ列挙耐性のため、既存アカウントかどうかで表示を変えない)
export function RequestSignupForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      // Server Action を呼ぶ。常に { ok: true } が返る (既存アカウントでも投げない)
    ...
    return (
      <div role="status" className="space-y-3 text-center">
        ...
        <p className="text-sm text-slate-600">
          <span className="font-medium text-slate-800">{email}</span> 宛にご案内をお送りしました。
          <br />
          メール内のリンクをクリックすると手続きを進められます。
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* 簡単な説明 */}
      <p className="text-sm text-slate-600">
        メールアドレスを入力すると、サインアップ用のリンクをお送りします。
      </p>
      {/* メールアドレス入力 */}
      <div>
