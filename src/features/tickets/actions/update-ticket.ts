...
// 担当者割当・エスカレーションのメール本文を生成する純粋ヘルパー (Phase 2)
import { renderAssignedEmail, renderEscalatedEmail, buildTicketUrl } from '@/lib/ticket-email';
...
// ステータス変更・優先度変更を依頼者へ複数チャネル (メール・LINE) で届ける通知ヘルパー
// (コードベース監査: update-ticket.ts の肥大化を解消するため notify/ モジュールへ分離した)
import {
  notifyRequesterOfStatusChange,
  notifyRequesterOfPriorityChange,
} from '@/features/tickets/notify/ticket-change-notifications';
