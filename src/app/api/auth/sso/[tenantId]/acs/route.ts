...
//   署名・Issuer・Audience・期限を検証し、本人のメールでテナント内のユーザーを特定する。
//   セッション発行は実績あるマジックリンク経路を再利用する: ワンタイムトークンを 1 件発行し、
//   その token を持つ確認ページ (200 HTML) を返す。ここではリダイレクトしない。
//   トークンを実際に消費してセッションを発行するのは、マジックリンクのコールバックの
//   **POST** ハンドラだけ (GET は消費せず、もう 1 枚確認ページを返す = メールゲートウェイの
//   プリフェッチ対策)。したがって利用者はこの確認と合わせて 2 回ボタンを押すことになる。
//
// セキュリティ:
...
// 遷移する。**この GET はトークンを消費せず、セッションも発行しない** —— 実際の発行は
// そのコールバックが返す 2 枚目の確認ページからの POST で行われる (プリフェッチ対策のため
// GET を副作用なしに保っている)。token は HTML 属性として安全にエスケープして埋め込む。
