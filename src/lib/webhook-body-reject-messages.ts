...
// export できず、置いたままではテストから参照できない。ここへ出す理由は
// 「**本番の表そのものを表明できるようにする**」こと:
//   - ルート越しに表を確かめられるのは `too-large` (413 を返させるのが容易) までで、
//     `timeout` / `unreadable` を発火させるには実時間で待つか接続を切る必要があり重い。
//   - ここへ出しておけば、ルートを経由せずに表そのもの＋ヘルパーの組で全理由を表明できる。
// なお「理由ごとに引く」実装をステータスの 2 値へ戻す退行**そのもの**は、合成の表を使う
// `tests/body-reject-response.test.ts` でも検出できる。こちらが受け持つのは、
// **本番の表が実際にその性質 (400 系の理由に別々の文言) を満たしているか**の方。
