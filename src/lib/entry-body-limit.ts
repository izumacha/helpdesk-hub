 ...
 *
 * **公開しているのはテストが導出の形を検算するため。** テストが固定するのは
 * 「入口の枠 = 経路別上限の最大値 + この余白」であることと、この余白が
 * `ENTRY_OVER_LIMIT_MARGIN_MIN_BYTES` を**超えている**ことの 2 点で、1MB という値そのものは
 * 固定しない (リテラルまで縛ると、余裕を見直すだけで落ちる単なる変更検知になるため)。
 */
export const ENTRY_OVER_LIMIT_MARGIN_BYTES = 1024 * 1024;

/**
 * 余白が**超えていなければならない**大きさ (= Node のソケット読み取り 1 回ぶん / 64KB)。
 *
 * **テストが「余白が消えていないこと」を機械的に固定するために公開する。** 余白を 0 にすると
 * 「入口の枠 ≧ 各経路の枠」は成立したままなので、経路ごとの比較だけでは退行を捕まえられない
 * (上の docstring が説明している 413 到達不能のバグに静かに戻る)。
 *
 * **必要条件は「ソケット読み取り 1 回分以上」。** 入口は枠を跨ぐチャンクを*まるごと*捨てて
 * 閉じるので、枠を E・チャンクサイズを c とするとルートに届く量 D は `E - c < D ≦ E` に収まる。
 * E = 経路の上限 R + 余白 なので、D の下限は `R + 余白 - c` を*超える*。したがって
 * 余白 ≧ c であれば D > R が保証され、`累計 + 次のチャンク > 上限` の超過判定が必ず成立する。
 *
 * **テストが「超える」で検査しているのは、それより 1 段厳しい側へ倒す意図的な保守**である
 * (境界ちょうどを許すと、将来 c の見積もりが 1 バイトでも甘かったときに保証が崩れる)。
 * 実際の余白 1MB は c = 64KB に対して十分厚いので、この差が効く場面は現状ない。
 */
