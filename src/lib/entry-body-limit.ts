...
// (nginx なら `location` ごとの `client_max_body_size`)。これは
 ...
 *
 * 余白を 1MB にしているのは、Node のソケット読み取りが highWaterMark (64KB) 単位で、
 * 捨てられる「枠を跨ぐチャンク」が必ずこの中に収まるため (64KB でも足りるが、将来
 * highWaterMark が変わっても効くよう桁で余裕を取る)。経路の上限 51MB に対して約 2% で、
 * メモリの見積もりを実質的に動かさない。
 */
