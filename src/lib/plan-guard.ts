...
// フォローアップ (監査で発見したギャップ): 新しい組織 (テナント) を追加作成できる機能
// (createTenant) の利用可否。有料プラン (Free 以外) のみ許可する。
//
// 重要: 引数には呼び出し元テナントの **生の (raw) subscriptionPlan** を渡すこと。
// 他のゲート関数の多くは `resolveEffectivePlan()` で §7.2 トライアル期間中の実効プラン
// (Free → Standard 相当に昇格) を解決してから渡す使い方を想定しているが、この関数は
// 意図的にその昇格を経由しない生の plan を受け取る契約にしている。理由: 30 日間の
// Free trial 中の管理者 (raw plan は 'free' のまま) が『新しい組織を作成する』を使うと、
// 作成された新テナントもまた raw plan 'free' + 新しい 30 日トライアルを持つため、
// 「トライアル管理者としてログインし直しては新規組織を作る」を繰り返すことで
// 無制限にトライアルを連鎖させられてしまう (実際に課金しているテナントの管理者だけが
// この機能を使える、という抑止力にする)。
export function isAdditionalTenantCreationAllowed(rawPlan: SubscriptionPlan): boolean {
  // Free (トライアル中を含む) 以外の実際に課金しているプランのみ許可する
  return rawPlan !== 'free';
}
