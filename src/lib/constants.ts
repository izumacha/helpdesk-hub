...
//
// なぜ受け皿が必要か: サーバーは Content-Length の申告だけでサイズ超過と分かる場合、本文を
// 1 バイトも読まずに 413 を返す (`request-body-limit.ts`)。このとき送信途中の接続が未消費のまま
// 閉じるため、ブラウザが応答より先に接続断を観測して fetch が reject することがある。
// catch が無いと例外がそのまま浮いて**画面に何も表示されない**ため、必ず捕まえて表示する。
