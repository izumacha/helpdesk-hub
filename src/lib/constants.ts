...
// 送信そのものが成立しなかったとき (オフライン・接続断・サーバーが本文を読まずに応答を返した等) に
// フォームへ出す文言。**fetch が reject した場合の受け皿**で、サーバーが返したエラー文言
// (`error` / `issues[0].message`) が読めた場合はそちらを優先する。
//
// なぜ 1 箇所に集約するか: この受け皿を必要とするフォームは複数あり (新規起票 / コメント投稿)、
// 各コンポーネントに直書きすると片方だけ推敲されてずれる (§6 UI 文言は単一の参照元に集約する)。
//
// なぜ受け皿が必要か: サイズ超過の大きいアップロードでは、送信途中の接続が未消費のまま閉じ、
// ブラウザが応答より先に接続断を観測して fetch が reject することがある。catch が無いと例外が
// そのまま浮いて**画面に何も表示されない**ため、必ず捕まえて表示する。
// 切るのは**アプリの手前のリバースプロキシ**（`docs/security.md` §7 の `client_max_body_size`）。
// アプリ側の `request-body-limit.ts` にも Content-Length だけで打ち切る事前検査はあるが、
// `src/proxy.ts` を置いている現状では入口の複製が先に本文を受け取り終えてからルートが起動する
// ため（`src/lib/entry-body-limit.ts`）、**アプリ層の 413 が未消費の本文を残すことはない**。
export const NETWORK_ERROR_MESSAGE =
  '送信できませんでした。通信状態をご確認のうえもう一度お試しください';

// 2xx は返ってきたが応答を解釈できず、登録できたのかどうか判断できないときの文言。
// **成功とも失敗とも言い切らない**のが要点: 起票 API は成功時に必ず id を含む JSON を返すので、
// 読み取れない 2xx は「アプリ以外 (プロキシ等) が応答した」可能性が高く、実際には登録されて
// いない公算が大きい。一方で登録済みの可能性も 0 ではないため、確認を挟んでから再送するよう促す
// (「もう一度お試しください」とだけ書くと重複チケットを生む)。
// コメント投稿版。こちらは一覧ではなく「画面を更新して確認」へ誘導する
// (投稿先はいま開いているチケットなので、確認する場所が違う)
export const COMMENT_RESULT_UNKNOWN_MESSAGE =
  '送信できたかどうかを確認できませんでした。画面を更新してご確認のうえ、反映されていなければもう一度お試しください';

export const TICKET_RESULT_UNKNOWN_MESSAGE =
  '登録できたかどうかを確認できませんでした。一覧をご確認のうえ、登録されていなければもう一度お試しください';
