...
// 登録そのものは成功したが、応答を読めず詳細ページへ遷移できなかったときの文言。
// **「失敗しました」とだけ出さない**のが要点で、そう出すと利用者が同じ内容をもう一度登録し、
// 重複チケットが生まれる。既に登録済みであることを伝えたうえで一覧へ誘導する。
export const TICKET_CREATED_BUT_NAVIGATION_FAILED_MESSAGE =
  '登録は完了しましたが、画面の切り替えに失敗しました。一覧から確認してください';
